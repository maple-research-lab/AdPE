...
def _rp_2d_product( diff, **kwargs):
    ...
    pid = piecewise_index(diff, **kwargs) #+beta, no shift directly normalized to [-1,1]
...
class iRPE(nn.Module):
    ...
    def forward(self, x, height=None, width=None):
        ...
        """
        rp_bucket = self._get_rp_bucket(x, height=height, width=width)
    ...
    def forward_rpe_transpose(self, x, rp_bucket):
        ...
        rp_buckets_mask = torch.clamp(rp_bucket, min=-self.rpe_config.beta, max=self.rpe_config.beta)
        ...
        #_,_,L_out = mask_rij_sample.shape
        #then multiply with x: B, H, L, head_dim
        #x_input = x.permute(1,2,0,3) #H,L,B,head_dim
        x_input = x.permute(1,2,0,3) #H,L,B,head_dim
        #r_h,r_hdim,L,L
        mask_allrij_sample = mask_allrij_sample.permute(0,2,1,3)#H,L,head_dim,L_out

        #x.transpose(0, 1).reshape(-1, B * L, self.head_dim)#H, B*L, head_dim
        #H,H_dim, L1, L2 to H,L1, h_dim, L2
