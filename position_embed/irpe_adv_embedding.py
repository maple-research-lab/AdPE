...
class iRPE(nn.Module):
    ...
    def __init__(self, head_dim, num_heads=8,
        ...
        self.adv_token = nn.Parameter(
                    torch.zeros(self.num_heads,self.head_dim,1))
    ...
    def forward(self, x, mask,height=None, width=None):
        ...
        if self.transposed:
            return self.forward_rpe_transpose(x,mask, rp_bucket)
    ...
    def forward_rpe_transpose(self, x,mask, rp_bucket):
        ...
        if self.mode == 'bias':
            ...
            lookup_table = self.lookup_table_weight#H,H_dim,num_buckets
            H,H_dim,num_buckets = lookup_table.shape
            # C, num_buckets,
            # mask: B*L
            lookup_table_adv = self.lookup_table_weight+self.adv_token.repeat(1,1,num_buckets)
            # x: B,L,num_heads,head_dim
            # lookup_table: num_heads,head_dim, (2beta+1)^2
            lookup_table = torch.matmul(
                x.transpose(0, 1).reshape(-1, B * L_query, self.head_dim),
                lookup_table).\
                view(-1, B, L_query, self.num_buckets).transpose(0, 1)
            # lookup_table_onlyadv = torch.matmul(
            #     x.transpose(0, 1).reshape(-1, B * L_query, self.head_dim),
            #     self.adv_token).\
            #     view(-1, B, L_query, 1).transpose(0, 1)# B*H*L*1
            #lookup_table_onlyadv = lookup_table_onlyadv.repeat(1,1,1,num_buckets)
            #lookup_table_adv = lookup_table_onlyadv +lookup_table
            lookup_table_adv = torch.matmul(
                x.transpose(0, 1).reshape(-1, B * L_query, self.head_dim),
                lookup_table_adv).\
                view(-1, B, L_query, self.num_buckets).transpose(0, 1)
            # B,H, L,num_bucket
            tmp_mask = mask.unsqueeze(1).repeat(1,H,1).unsqueeze(-1).repeat(1,1,1,self.num_buckets)
            lookup_table= tmp_mask*lookup_table_adv+(1-tmp_mask)*lookup_table
            if RPEIndexFunction is not None:
                lookup_table = RPEIndexFunction.apply(lookup_table, rp_bucket)
            else:
                lookup_table= lookup_table.flatten(2)[:, :, self._ctx_rp_bucket_flatten].\
                    view(B, -1, L_query, L_key)
